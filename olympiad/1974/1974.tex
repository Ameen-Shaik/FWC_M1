...
\begin{center}
{\Large \textbf{Sixteenth International Olympiad, 1974}}
\end{center}
...
$$
\sin A \sin B \leq \frac{\sin^2 C}{2}.
$$
...
Prove that the number $\sum_{k=0}^{n} \binom{2n+1}{2k+1}^{2} 3^k$ is not divisible by 5 for any integer $n \geq 0$.
...
\item[(ii)] If $a_i$ is the number of white squares in the $i$-th rectangle, then $a_1<a_2<\cdots<a_p$. Find the maximum value of $p$ for which such a decomposition is possible. For this value of $p$, determine all possible sequences $a_1,a_2,\cdots,a_p$.
\end{enumerate}
...
Let $P$ be a non-constant polynomial with integer coefficients. If $n(P)$ is the number of distinct integers $k$ such that $(P(k))^2=1$, prove that $n(P)-\deg(P)\leq2$, where $\deg(P)$ denotes the degree of the polynomial $P$.
